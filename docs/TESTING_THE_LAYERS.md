...
docker inspect "$C" -f '{{range .Config.Env}}{{println .}}{{end}}' | grep MODEL_API_KEY
```
Verified output:
```
MODEL_API_KEY=ark-run-nPR0J8DB...        <- a run-scoped broker token
```

`MODEL_API_KEY` is the provider-neutral name the broker token is injected under, for both
GLM and Ark. The real key never appears under any name.
...
```bash
KEY=$(grep -oP '(?<=^GLM_API_KEY=).*' .env)   # or ARK_API_KEY when MODEL_PROVIDER=ark
