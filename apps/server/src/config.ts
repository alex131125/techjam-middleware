 ...
 *
 * `base_url` points at the credential broker rather than at the model provider directly,
 * and `env_key` resolves to a run-scoped token rather than the real key, so the Runtime
 * holds no durable credential. See middleware/broker.ts.
 *
 * The provider block is derived from the selected provider (GLM or Ark), so switching
 * MODEL_PROVIDER does not weaken the broker indirection.
 */
export async function writeCodexConfig(
  ...
  const toml = [
    ...
    "name = " + JSON.stringify(provider.name + " (via Launchpad credential broker)"),
