...
describe("Container Codex runner", () => {
  it("builds an isolated Docker/Podman-compatible invocation", () => {
    ...
    // The run-scoped token is passed BY VALUE. The bare `--env MODEL_API_KEY`
    // passthrough form would hand the Runtime the control plane's real provider key,
    // which is the exact leak L2 exists to close.
  ...
  it("places the Runtime on the internal network and withholds the provider key", () => {
    const config = loadConfig({
      NODE_ENV: "test",
