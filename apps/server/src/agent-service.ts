...
import { agentCodexHome, isArkConfigured, writeCodexConfig } from "./config.js";
...
export class AgentService {
  ...
  constructor(
    ...
    this.secrets.register(config.arkApiKey);
    ...
    this.broker = new ArkBroker({
      ...
      onCall: (record) => {
        this.recorder.record(
          ...
          { status: record.status, durationMs: record.durationMs, path: record.path },
        );
      },
    });
  }
  ...
  async createAgent(input: CreateAgentInput): Promise<Agent> {
    ...
    const agent: Agent = {
      ...
      codexThreadId: null,
      lastError: null,
      budgetPolicy: input.budgetPolicy ? this.validatePolicy(input.budgetPolicy) : null,
      createdAt: timestamp,
      updatedAt: timestamp,
    };
  ...
  async deleteAgent(id: string): Promise<{ archivedWorkspace: string }> {
    ...
    await this.store.mutate((database) => {
      ...
      database.violations = database.violations.filter((item) => item.agentId !== id);
    });
  ...
  getTrace(runId: string): TraceEvent[] {
    ...
    const persisted = this.store.snapshot().traces.filter((event) => event.runId === run.id);
  ...
  getViolations(agentId?: string): PolicyViolation[] {
    ...
    const scoped = agentId ? violations.filter((item) => item.agentId === agentId) : violations;
  ...
  ceiling(): CapabilityBudget {
    return platformCeiling(this.config.arkBaseUrl, this.config.maxStepsPerRun);
  }
  ...
  effectiveBudget(agent: Agent): CapabilityBudget {
    ...
    try {
      ...
      // A stored policy that no longer fits the ceiling is ignored rather than honoured,
      // which fails closed: the Agent gets the ceiling, never more.
      return ceiling;
    }
  }
   ...
   */
  async narrowAgentBudget(id: string, proposal: AgentBudgetPolicy): Promise<Agent> {
    const agent = this.getAgent(id);
  ...
  private validatePolicy(policy: AgentBudgetPolicy): AgentBudgetPolicy {
    try {
      const narrowed = narrowStrict(this.ceiling(), policy);
      return {
        commandClasses: [...narrowed.commandClasses],
        egressAllowlist: [...narrowed.egressAllowlist],
        maxSteps: narrowed.maxSteps,
  ...
  async systemInfo(): Promise<Record<string, unknown>> {
    ...
    return {
      ...
      middleware: {
        ...
        egressAllowlistEnforced: degraded.every((item) => item.control !== "egress-allowlist"),
        runtimeNetwork:
          this.config.runtimeProvider === "container" ? this.config.runtimeNetwork : null,
  ...
  private async executeRun(agentAtStart: Agent, run: AgentRun): Promise<void> {
    ...
    const budget: FrozenBudget = freeze(
      ...
      agentAtStart.id,
      run.id,
      agentAtStart.budgetPolicy ? ["platform-ceiling", "agent-policy"] : ["platform-ceiling"],
    );
    const brokerToken = this.broker.issue(agentAtStart.id, run.id);
    ...
    this.recorder.record(agentAtStart.id, run.id, "budget.frozen", "Capability budget frozen", {
    ...
    const onEvent = (event: RunnerEvent): void => this.onRunnerEvent(agentAtStart, run, event, violations);
  ...
  private onRunnerEvent(
    ...
    switch (event.kind) {
      case "command.started":
        this.recorder.record(agent.id, run.id, "command.started", event.command, {
      ...
      case "command.completed":
        ...
        return;
      case "file.change":
        this.recorder.record(agent.id, run.id, "file.change", "Workspace file change", event.detail);
        return;
      case "reasoning":
        this.recorder.record(agent.id, run.id, "reasoning", event.text, { text: event.text });
        return;
      case "agent.message":
        this.recorder.record(agent.id, run.id, "agent.message", event.text, { text: event.text });
        return;
      case "policy.violation": {
        for (const detail of event.violations) {
          ...
          this.recorder.record(
            ...
            { kind: detail.kind, command: violation.command, terminal: event.terminal },
          );
        }
        return;
      }
