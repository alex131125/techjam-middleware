 ...
 */
export function platformCeiling(modelBaseUrl: string, maxSteps: number): CapabilityBudget {
  let modelHost = "";
  try {
    modelHost = normalizeHost(new URL(modelBaseUrl).hostname);
  } catch {
    modelHost = "";
  }
  return {
    ...
    egressAllowlist: modelHost ? [modelHost] : [],
    maxSteps,
